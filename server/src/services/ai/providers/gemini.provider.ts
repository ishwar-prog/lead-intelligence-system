...
} from '../prompts/leadAnalysis.prompt';
...
export class GeminiProvider implements AIProvider {
  ...
  async analyzeLead(input: LeadAnalysisInput): Promise<LeadAnalysisOutput> {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        console.log(`[Gemini] Attempt ${attempt}/${MAX_RETRIES} for company: ${input.company}`);
        ...
        const response = await axios.post(
          `${this.apiUrl}?key=${env.GEMINI_API_KEY}`,
          {
            ...
            generationConfig: {
               ...
               */
              temperature: 0.2,
              maxOutputTokens: 1500, // Hard cap — prevents runaway expensive responses
              topP: 0.8,
            },
          },
          {
